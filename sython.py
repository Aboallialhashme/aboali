...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@QIQlQII"))
...
ownerhson_id = 5807751718
@sython.on(events.NewMessage(outgoing=False, pattern='فزعه'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك ميرو ابشر  @QIQlQII')
...
@sython.on(events.NewMessage(outgoing=True, pattern=r"\.تجميع العرب"))
async def _(event):
        await event.edit("حسنا, تأكد من انك مشترك ب قنوات الاشتراك الاجباري لتجنب الأخطأء")
        channel_entity = await sython.get_entity(bot_usernamee)
        await sython.send_message('@xnsex21bot', 'جاري التجميع بواسطة | SOMY TEAM')
        channel_entity = await sython.get_entity(bot_usernamee)
        await sython.send_message('@xnsex21bot', '/start')
        await asyncio.sleep(5)
        msg0 = await sython.get_messages('@xnsex21bot', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(5)
        msg1 = await sython.get_messages('@xnsex21bot', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await sython(ImportChatInviteRequest(bott))
                msg2 = await sython.get_messages('@xnsex21bot', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
...
async def _(event):
      await event.reply("""السـورس يعمـل |ميرو
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍
...
async def _(event):
      await event.reply("""ميرو : @QIQlQII"""
)
